...
class Test():
    ...
    def __init__(self,
        ...
        self.__tracking_top_n = tracking_top_n
        self.__mask_y = None
    ...
    def fit(self, X, y):
        ...
        with self.__graph.as_default():
            ...
            with self.__sess.as_default():
                ...
                # save
                self.__saver = [tf.train.Saver(tf.global_variables(), max_to_keep=1) for i in range(self.__tracking_top_n)]
                ...
                convergence_count = 0
                # min -> max
                top_score = [-99999 + i for i in range(self.tracking_top_n)]
                for batch in batches:
                    ...
                    if self.__evaluate:
                        if current_step % self.__evaluate_every == 0:
                            print("\nEvaluation:")
                            current_loss, BOTscore = self.__dev_step(x_dev, y_dev)
                            print("")
                            if BOTscore >= min_score[0]:
                                for i, v in reversed(enumerate(top_score)):
                                    if BOTscore >= v:
                                        convergence_count = 0
                                        if i > 0:
                                            top_score[0:i - 1] = top_score[1:i]
                                            top_score[i] = BOTscore
                                        else:
                                            top_score[i] = BOTscore
                                        checkpoint_prefix = os.path.join(self.__out_dir, '{}_{:.4}_{:.4}'.format(current_step, current_loss, BOTscore))
                                        path = self.__saver[i].save(self.__sess, checkpoint_prefix, global_step=current_step)
    ...
    def decision_function(self, X):
        ...
        all_scores = False
        print([n.name for n in self.__graph.get_operations() if 'fc2' in n.name or 'predictions' in n.name])
