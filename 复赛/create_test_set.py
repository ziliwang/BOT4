    ...
    if len(MAP_INDEX_KEY.keys()) > 0:
        for i, v in enumerate(header):
            ...
            if MAP_INDEX_KEY[i] != v:
                return False
            if len(check.keys()) < len(MAP_INDEX_KEY.keys()) + 2:
                check[v] = []
    else:
        for i, v in enumerate(header):
            if i == 0:
...
def process(folder):
    # check
    if not set(['test_labs.txt', 'test_index_labs.txt']).issubset(set(os.listdir(folder))):
        raise(ValueError('illegal folder'))
    # index
    one_type_process(lab_file=os.path.join(folder, 'test_index_labs.txt'),
                     image_dir=os.path.join(folder, 'index_pic'),
                     _type='index')
    # gegu
    one_type_process(lab_file=os.path.join(folder, 'test_labs.txt'),
...
def main(data, height, width, name):
    global HEIGH
    global WIDTH
    global OUT_FOLDER
    HEIGH = int(height)
    WIDTH = int(width)
    OUT_FOLDER = os.path.join('train_cal', '{}_{}_{}'.format(name, HEIGH, WIDTH))
