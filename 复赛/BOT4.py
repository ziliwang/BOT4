    ...
    def __init__(self, l2_reg_lambda=0.001):
        ...
        with slim.arg_scope(
            ...
            with tf.name_scope("top"):
                ...
                branch10x100 = slim.conv2d(branch1x1, 96, [160, 11],
            ...
            with tf.name_scope("bottom"):
                ...
                branch10x100 = slim.conv2d(branch1x1, 32, [55, 11],
                                           stride=6, padding='VALID',
                                           scope="bottom-55x11")
                # pool layer -> 1x15x32
        ...
        with tf.name_scope("loss"):
            losses = tf.sqrt(tf.square(self.predictions - self.input_y) + 1e-15) - 0.1 * tf.sign(self.predictions * self.input_y)
            self.loss = tf.reduce_sum(losses) + tf.reduce_sum(slim.losses.get_regularization_losses())
