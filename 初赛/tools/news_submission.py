...
class news_submission(list):
                    ...
                    try:
                        if isinstance(sampledata[i], dict):
                            ...
                            for k in range(len(annoncedata)):
                                if annoncedata[k]['news_id']==samplitem.news_id:
    ...
    def load_news(self, relationdata, sampledata):
        """
        将投研报告对应至评分时点单位
        :param self:
        :param relationdata: 投研报告关系檔
        ...
        try:
            if isinstance(relationdata, list) and isinstance(sampledata, list):
                print('开始加载新闻NewsTrainSample')
                tot = len(sampledata)
                for i in range(len(sampledata)):
                    try:
                        if isinstance(sampledata[i], dict):
                            samplitem=NewsTrainSample(sampledata[i])
                            for k in range(len(relationdata)):
                                if relationdata[k]['news_id']==samplitem.news_id:
                                    relationitem=NewsRelations(relationdata[k])
                                    uuid=self._lookup_uuid(relationitem.security_id,relationitem.publish_date)
                                    if uuid is None:
                                        print(relationdata[k])
                                    else:
                                        self.insert_news(samplitem,uuid)
                    except OSError as e:
                        print(e)
    ...
    def insert_news(self,news_sample, v):
        if isinstance(news_sample, AnnouncementsTrainSample):
            news_sample.notice_date = None
            news_sample.publish_date = None
            v.announcelist.append(news_sample)
        elif isinstance(news_sample, ResearchTrainSample):
            v.researchlist.append(news_sample)
        elif isinstance(news_sample, NewsTrainSample):
            v.newslist.append(news_sample)

    def _lookup_uuid(self, security_id,data_date):
        if not self.is_masked:
            for v in self:
                if v.security_id==security_id and time.strftime("%Y-%m-%d", v.data_date)==data_date:
                    return v
    def mask_data(self):
        if not self.is_masked:
...
class ResearchRelations(object):
    ...
    def __init__(self, jsondict):
        if isinstance(jsondict, dict):
            ...
            self.publish_date = jsondict['publish_date'].replace('T00:00:00','')
            self.uuid=None
